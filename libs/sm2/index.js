...
const encodeDer = asn1.encodeDer;
const decodeDer = asn1.decodeDer;
const SM3Digest = sm3.SM3Digest;
const SM2Cipher = sm2.SM2Cipher;

// const generateEcparam = utils.generateEcparam;
...
function doEncrypt(msg, publicKey, cipherMode = 1) {
  ...
  msg = utils.hexToArray(utils.parseUtf8StringToHex(msg))
...
function doDecrypt(encryptData, privateKey, cipherMode = 1) {
  ...
  const isDecrypt = utils.arrayToHex(c3utils) === c3

  if (isDecrypt) {
    const decryptData = utils.arrayToUtf8(data)
    return decryptData
  } else {
    return ''
  }
...
function doSignature(msg, privateKey, {
  ...
  let hashHex = typeof msg === 'string' ? utils.parseUtf8StringToHex(msg) : utils.parseArrayBufferToHex(msg)
...
function doSm3Hash(hashHex, publicKey, userId = '1234567812345678') {
  ...
  const zValue = utils.hexToArray(utils.arrayToHex(z).toString())

  const p = hashHex
  const pValue = utils.hexToArray(p)
  ...
  return utils.arrayToHex(hashData).toString()
}
